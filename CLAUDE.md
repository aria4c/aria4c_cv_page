...
- **Mobile (narrow ≤720px).** Full pattern: **`src/MOBILE_LAYOUT.spec.md`** (`MobileTouchNav` touch strip, footer hidden, title-bar hints suppressed, status bar density). Never show **two** visible Back/Home/theme affordances; keyboard handlers may still register via `BackPrompt` / `BBSRoot`.
...
- **Spec files live next to implementation.** `*.spec.md` alongside the file it describes. Cross-cutting mobile rules: `src/MOBILE_LAYOUT.spec.md`.
